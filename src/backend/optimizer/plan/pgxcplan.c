									...
									List **unshippable_quals, List **rep_tlist)
{
	 ...
	 */
	extract_actual_join_clauses(rqpath->join_restrictlist, rqpath->path.parent->relids, &join_clauses,
									&other_clauses);
	...
	if (cmdtype == CMD_UPDATE)
	{
		...
		for (attnum = 1; attnum <= natts; attnum++)
		{
			...
			pgxc_add_param_as_tle(query_to_deparse, attnum,
								...
								get_attname(res_rel->relid, attnum, false));
		}
...
create_remotegrouping_plan(PlannerInfo *root, Plan *local_plan)
{
	...
	if (sort_plan)
	{
		...
		sort_plan = make_sort_from_groupcols(query->groupClause,
...
create_remotelimit_plan(PlannerInfo *root, Plan *local_plan)
{
	...
	else if (local_limit->limitCount)
	{
		...
		else
		{
			...
			rq_limitCount = (Node *)make_op(pstate,
