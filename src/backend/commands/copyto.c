...
#ifdef PGXC
static RemoteCopyOptions *GetRemoteCopyOptions(CopyToState cstate);
#endif
/*
...
DoCopyTo(CopyToState cstate)
{
	 ...
	 */
	if (cstate->opts.binary && IS_PGXC_COORDINATOR)
#else
	if (cstate->opts.binary)
#endif
	{
...
GetRemoteCopyOptions(CopyToState cstate)
{
	RemoteCopyOptions *res = makeRemoteCopyOptions();
	Assert(cstate);

	// Then fill in structure
	res->rco_binary = cstate->opts.binary;
	res->rco_csv_mode = cstate->opts.csv_mode;
	if (cstate->opts.delim)
		res->rco_delim = pstrdup(cstate->opts.delim);
	if (cstate->opts.null_print)
		res->rco_null_print = pstrdup(cstate->opts.null_print);
	if (cstate->opts.quote)
		res->rco_quote = pstrdup(cstate->opts.quote);
	if (cstate->opts.escape)
		res->rco_escape = pstrdup(cstate->opts.escape);
	if (cstate->opts.force_quote)
		res->rco_force_quote = list_copy(cstate->opts.force_quote);
	if (cstate->opts.force_notnull)
		res->rco_force_notnull = list_copy(cstate->opts.force_notnull);
