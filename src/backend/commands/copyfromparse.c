...
#ifdef PGXC
static void CopySendChar(CopyFromState cstate, char c);
static void CopySendInt32(CopyFromState cstate, int32 val);
static void CopySendData(CopyFromState cstate, const void *databuf, int datasize);
 ...
 */
static void
append_defvals(Datum *values, CopyFromState cstate)
{
	CopyFromStateData new_cstate = *cstate;
	int i;
	...
	for (i = 0; i < cstate->num_defaults; i++)
	{
		...
		if (!cstate->opts.binary)
			CopySendChar(&new_cstate, new_cstate.opts.delim[0]);
		...
		if (cstate->opts.binary)
		{
			...
			outputbytes = SendFunctionCall(&cstate->in_functions[attindex], defvalue);
		...
		else
		{
			...
			string = OutputFunctionCall(&cstate->in_functions[attindex], defvalue);
			if (cstate->opts.csv_mode)
...
#ifdef PGXC
static void
CopySendChar(CopyFromState cstate, char c)
{
 ...
 */
static inline void
CopySendInt32(CopyFromState cstate, int32 val)
{
	uint32		buf;
...
static void
CopySendString(CopyFromState cstate, const char *str)
{
 ...
 */
static void
CopySendData(CopyFromState cstate, const void *databuf, int datasize)
{
...
static void
CopyAttributeOutText(CopyFromState cstate, const char *string)
{
	const char *ptr;
	const char *start;
	char		c;
	char		delimc = cstate->opts.delim[0];
	bool encoding_embeds_ascii = PG_ENCODING_IS_CLIENT_ONLY(cstate->file_encoding);
 ...
 */
static void
CopyAttributeOutCSV(CopyFromState cstate, const char *string,
					bool use_quote, bool single_attr)
{
	const char *ptr;
	const char *start;
	...
	if (!use_quote)
	{
		...
		else
		{
			...
			while ((c = *tptr) != '\0')
			{
				...
				if (IS_HIGHBIT_SET(c) && encoding_embeds_ascii)
