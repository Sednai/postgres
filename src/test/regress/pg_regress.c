...
stop_gtm(void)
{
	...
	snprintf(buf, sizeof(buf),
			 SYSTEMQUOTE "\"%s%sgtm_ctl\" stop -Z gtm -D \"%s/%s\" -m fast" SYSTEMQUOTE,
			 bindir ? bindir : "",
...
stop_node(PGXCNodeTypeNum node)
{
	...
	snprintf(buf, sizeof(buf),
			 SYSTEMQUOTE "\"%s%spg_ctl\" stop -D \"%s/%s\" -s -m fast" SYSTEMQUOTE,
			 bindir ? bindir : "",
			 bindir ? "/" : "", temp_instance, data_folder);
	r = system(buf);
	if (r != 0)
...
start_node(PGXCNodeTypeNum node, bool is_coord, bool is_main)
{
	...
	if (node == PGXC_GTM)
	{
		/* Case of a GTM start */
		snprintf(buf, sizeof(buf),
				 SYSTEMQUOTE "\"%s%sgtm\" -D \"%s/%s\" -p %d -x 10000 > \"%s/log/gtm.log\" 2>&1" SYSTEMQUOTE,
				 bindir ? bindir : "",
	...
	else
	{
		...
		if (is_main)
			snprintf(buf, sizeof(buf),
					 SYSTEMQUOTE "\"%s%spostgres\" %s -i -p %d -k %s -D \"%s/%s\"%s -c \"listen_addresses=%s\" > \"%s/log/postmaster_%d.log\" 2>&1" SYSTEMQUOTE,
					 bindir ? bindir : "",
					 bindir ? "/" : "",
			...
			snprintf(buf, sizeof(buf),
					 SYSTEMQUOTE "\"%s%spostgres\" %s -i -p %d -k %s -D \"%s/%s\"%s > \"%s/log/postmaster_%d.log\" 2>&1" SYSTEMQUOTE,
					 bindir ? bindir : "",
			 		 bindir ? "/" : "",
					 is_coord ? "--coordinator" : "--datanode",
					 port_number,
...
initdb_node(PGXCNodeTypeNum node)
{
	...
	{
		snprintf(buf, sizeof(buf),
				 SYSTEMQUOTE "\"%s%sinitgtm\" -Z gtm -D \"%s/%s\" --noclean%s > \"%s/log/initgtm.log\" 2>&1" SYSTEMQUOTE,
				 bindir ? bindir : "",
			 	 bindir ? "/" : "", temp_instance, data_folder,
				 debug ? " --debug" : "",
				 outputdir);
	...
	{
		snprintf(buf, sizeof(buf),
				 SYSTEMQUOTE "\"%s%sinitdb\" --nodename %s -D \"%s/%s\" --noclean%s%s > \"%s/log/initdb.log\" 2>&1" SYSTEMQUOTE,
				 bindir ? bindir : "",
