...
SELECT c AS cidr, i AS inet FROM INET_TBL ORDER BY cidr,inet;

-- now test some support functions

SELECT i AS inet, host(i), text(i), family(i) FROM INET_TBL ORDER BY inet,host;
SELECT c AS cidr, abbrev(c) FROM INET_TBL;
SELECT c AS cidr, broadcast(c),
  i AS inet, broadcast(i) FROM INET_TBL ORDER BY 2,4;
SELECT c AS cidr, network(c) AS "network(cidr)",
  i AS inet, network(i) AS "network(inet)" FROM INET_TBL ORDER BY 2,4;
SELECT c AS cidr, masklen(c) AS "masklen(cidr)",
  i AS inet, masklen(i) AS "masklen(inet)" FROM INET_TBL ORDER BY 2,4;

SELECT c AS cidr, masklen(c) AS "masklen(cidr)",
  i AS inet, masklen(i) AS "masklen(inet)" FROM INET_TBL
  WHERE masklen(c) <= 8 ORDER BY inet;

SELECT c AS cidr, i AS inet FROM INET_TBL
  WHERE c = i ORDER BY cidr;
...
SELECT set_masklen(inet(text(i)), 24) FROM INET_TBL ORDER BY 2;
...
SELECT * FROM inet_tbl WHERE '192.168.1.0/24'::cidr >>= i ORDER BY c,i;
-- EXPLAIN (COSTS OFF)
-- SELECT * FROM inet_tbl WHERE '192.168.1.0/24'::cidr >> i;
SELECT * FROM inet_tbl WHERE '192.168.1.0/24'::cidr >> i ORDER BY c,i;
